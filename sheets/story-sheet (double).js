...
export class StorySheetDouble extends DocumentSheet {
    pageFlip = "modules/storyteller/sounds/paper-flip.mp3"

    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            baseApplication: 'JournalSheet',
            classes: ["sheet", "story-sheet-double", game.settings.get('storyteller', 'theme')],
            template: 'modules/storyteller/templates/story-sheet (double).html',
            width: getBookWidth(),
    ...
    _onConfigureStory(event) {
        let story = $(event.target).closest(".window-app.story-sheet-double")
